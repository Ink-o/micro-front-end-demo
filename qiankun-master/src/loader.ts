...
function createElement(
  ...
  // 作用域css，就是拿到所有的style标签里面的css 增加前缀(对于link而言，后面会将link也转换成style)
  if (scopedCSS) {
...
export async function loadApp<T extends ObjectType>(
  ...
  const {
    singular = false, // 是否为单例，默认值为 false
    sandbox = true,
    excludeAssetFilter,
  ...
  // execScripts 在沙箱中运行 默认在执行前，需要给 global （假的window，子应用的window）扩展自定义的属性
  const {
    beforeUnmount = [],
    afterUnmount = [],
    afterMount = [],
