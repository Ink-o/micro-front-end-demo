...
const autoDowngradeForLowVersionBrowser = (configuration: FrameworkConfiguration): FrameworkConfiguration => {
  ...
  if (sandbox) {
    if (!window.Proxy) { // 不支持 proxy，则采用快照沙箱
  ...
  unregisteredApps.forEach((app) => {
    ...
    registerApplication({
      ...
      app: async () => {
        ...
        // loadApp 返回的是一个Promise函数，这里还需要等待 loadApp 返回的函数执行完毕
...
// 注册和start采用的都是single-spa
// 只是在 single-spa 上增加 预加载和沙箱功能
// 预加载功能：利用空闲时间进行加载其他应用
// 沙箱功能：css沙箱（创建一个sanbox，让你的execScript方法运行在sanbox）样式隔离（影子dom，scopedcss）
// 获取导出的接入协议（在沙箱中执行的）进行拓展（增加了用户提供的声明周期）
export function start(opts: FrameworkConfiguration = {}) {
  // 在 start 参数中，增加了 prefetch(预加载) singular（单例模式）沙箱
  ...
  // 对沙箱来做降级处理，有的沙箱不支持 proxy
  frameworkConfiguration = autoDowngradeForLowVersionBrowser(frameworkConfiguration);

  startSingleSpa({ urlRerouteOnly }); // 本质上这个方法就是single-spa的方法
  started = true;
